...
###   
<br/>1\. Creating the initial `main.py` file for your Plugin.

Firstly you will need to name your Plugin. Let’s say that in this example I am just going to name it `ExamplePlugin`. Once you’ve got a name you can create the `Plugins/ExamplePlugin/main.py` file, this will be the main entrypoint for your Plugin.
...
**a)** The length of the wait time between keypresses `1/20` needs to be changed based on the framerate, as the game only listens to inputs between the frames.

**b)** The highbeams only work when the normal lights are enabled.
